 ...
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║                                                                              ║
 * ║   🎨 HUNYUAN IMAGE 3 - MEERA MAX INTIMATE COLLECTION 🎨                      ║
 * ║                                                                              ║
...
const OUTPUT_DIR = '/home/ecolex/version1/generated-meera-hunyuan3';
...
async function generateImage(variant, index, total) {
  ...
  try {
    ...
    // Handle output - Hunyuan returns array of FileOutput objects
    if (Array.isArray(output) && output.length > 0) {
      const fileOutput = output[0];
      // Get URL using the url() method
      const imageUrl = typeof fileOutput.url === 'function' ? fileOutput.url() : fileOutput.url || fileOutput;

      if (imageUrl && typeof imageUrl === 'string' && imageUrl.startsWith('http')) {
        const filename = `meera_hunyuan3_${variant.id}_${Date.now()}.webp`;
        ...
        // Try writing directly using writeFile
        const filename = `meera_hunyuan3_${variant.id}_${Date.now()}.webp`;
        const filepath = path.join(OUTPUT_DIR, filename);
        await writeFile(filepath, fileOutput);
        const size = (fs.statSync(filepath).size / 1024 / 1024).toFixed(2);
        console.log(`[${new Date().toLocaleTimeString()}]    ✅ ${filename} (${size} MB) [direct write]`);
        return { variant: variant.id, filename, size, success: true };
      }
    } else {
      console.log(`[${new Date().toLocaleTimeString()}]    ❌ No output received`);
      console.log(`[${new Date().toLocaleTimeString()}]    Raw:`, typeof output, JSON.stringify(output).substring(0, 150));
...
╚══════════════════════════════════════════════════════════════════════════════╝
  ...
  console.log(`[${new Date().toLocaleTimeString()}] 🎨 Model: Tencent Hunyuan Image 3`);
  console.log(`[${new Date().toLocaleTimeString()}] 🔓 Safety Checker: DISABLED\n`);
  ...
  console.log(`
╔══════════════════════════════════════════════════════════════════════════════╗
║          🎨 HUNYUAN IMAGE 3 MAX INTIMATE COMPLETE 🎨                         ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
`);
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      collection: 'Hunyuan Image 3 - Meera Max Intimate',
      model: MODEL,
      resolution: '768x1024',
      safetyChecker: 'disabled',

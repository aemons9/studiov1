...
// Get VertexAI credentials - uses sharedAuthManager with auto-refresh
async function getVertexAICredentialsAsync(): Promise<{ projectId: string; oauthToken: string }> {
  try {
    const [oauthToken, projectId] = await Promise.all([
      getOAuthToken(),
      getProjectId()
    ]);
    return { projectId, oauthToken };
  } catch (error) {
    // Fallback to localStorage if proxy not available
    const projectId = localStorage.getItem('vera_project_id') || localStorage.getItem('vertex_project_id');
    const oauthToken = localStorage.getItem('vera_oauth_token') || localStorage.getItem('vertex_oauth_token');

    if (!projectId || !oauthToken) {
      throw new Error('VertexAI credentials not configured. Please ensure the proxy server is running (npm run proxy) or manually set your Project ID and OAuth token in Vera settings.');
    }
...
// Now uses async credentials with auto-refresh support
async function vertexAIRequest(endpoint: string, body: any, retryOnAuth: boolean = true): Promise<any> {
  // Use async credentials fetch for auto-refresh
  const { projectId, oauthToken } = await getVertexAICredentialsAsync();
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Check for auth errors - try to refresh token and retry
      if ((response.status === 401 || response.status === 403) && retryOnAuth) {
        console.log('🔄 Auth error detected, attempting token refresh...');
        try {
          await forceRefreshToken();
          // Retry the request once with new token
          return vertexAIRequest(endpoint, body, false);
        } catch (refreshError) {
          throw new Error(`Authentication error: ${errorText}. Token refresh failed. Please check your OAuth token is valid and not expired.`);
        }
      }
...
export const generateVideo = async (prompt: string, onStatusUpdate: (status: string) => void): Promise<string> => {
  // Use async credentials with auto-refresh
  const { projectId, oauthToken } = await getVertexAICredentialsAsync();
